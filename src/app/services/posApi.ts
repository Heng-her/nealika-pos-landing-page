...
const POS_ACCESS_INFO_PATH = "/pos_access_info";
...
export async function getPosAccessInfo() {
  const data = await apiRequest<unknown>(POS_ACCESS_INFO_PATH, {
    requiresAuth: true,
  });
...
export async function regeneratePosAccessPassword() {
  return apiRequest<unknown>(`${POS_ACCESS_INFO_PATH}/regenerate_password`, {
    method: "POST",
    requiresAuth: true,
  });
}
...
export async function revealPosAccessPassword() {
  const data = await apiRequest<unknown>(
    `${POS_ACCESS_INFO_PATH}/reveal_password`,
    {
      method: "POST",
      requiresAuth: true,
    },
...
export async function createPosLoginTicket() {
  const data = await apiRequest<unknown>(`${POS_ACCESS_INFO_PATH}/login`, {
    method: "POST",
    requiresAuth: true,
  });
