import { Check, CreditCard, BarChart3, Users, Zap, Shield, X, User } from "lucide-react";
  ...
  const pricingPlans = [
    {
      ...
      features: ["Up to 1 register", "Basic inventory management", "Sales reporting", "Email support", "Mobile app access", "Payment processing"],
      highlighted: false,
    },
    {
      id: 2,
  ...
  const features = [
    {
      icon: CreditCard,
      title: "Fast Payments",
      description: "Accept all major payment methods with lightning-fast processing",
    },
    {
      icon: BarChart3,
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white">
      ...
      <header className="bg-white sticky top-0 z-50 shadow-sm">
        ...
        <div className="border-b border-slate-200">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-2">
            <div className="flex items-center justify-end gap-6 text-sm">
              <button onClick={() => setShowLoginPage(true)} className="text-slate-600 hover:text-slate-900 transition-colors flex items-center gap-1">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between">
            ...
            <nav className="flex items-center gap-6 py-[20px]">
              <a href="#" className="text-slate-700 hover:text-blue-600 transition-colors flex items-center gap-1">
                <span>🏠</span>
                Home
              </a>
              ...
              <a href="#" className="text-blue-600 font-medium relative">
                POS
                <span className="absolute -top-3 -right-6 px-1.5 py-0.5 bg-green-500 text-white text-[10px] font-semibold rounded">NEW</span>
              </a>
      ...
      <section className="relative py-48 md:py-64 px-4 sm:px-6 lg:px-8 overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto text-center">
          ...
          <p className="text-xl text-slate-200 mb-8 max-w-2xl mx-auto">
            Streamline your sales, manage inventory, and grow your business with our powerful point-of-sale solution
          </p>
      ...
      <section id="features" className="py-20 px-4 sm:px-6 lg:px-8 bg-white">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16">
            ...
            <p className="text-lg text-slate-600">Powerful features designed to help your business thrive</p>
          </div>
              ...
              <div
                ...
                {plan.highlighted && (
                  <div className="text-center mb-4">
                    <span className="inline-block px-4 py-1 bg-white/20 rounded-full text-sm font-semibold">Most Popular</span>
                  </div>
                )}
                <h3 className={`text-2xl font-bold mb-2 ${plan.highlighted ? "text-white" : "text-slate-900"}`}>{plan.name}</h3>
                <div className="mb-4">
                  <span className="text-5xl font-bold">${plan.price}</span>
                  <span className={`text-lg ${plan.highlighted ? "text-white/80" : "text-slate-600"}`}>{plan.period}</span>
                </div>
                <p className={`mb-6 ${plan.highlighted ? "text-white/90" : "text-slate-600"}`}>{plan.description}</p>
                <button
                  onClick={handleGetStarted}
                ...
                <div className="space-y-3">
                  {plan.features.map((feature, featureIndex) => (
                    <div key={featureIndex} className="flex items-start gap-3">
                      <Check className={`w-5 h-5 flex-shrink-0 mt-0.5 ${plan.highlighted ? "text-white" : "text-blue-600"}`} />
                      <span className={plan.highlighted ? "text-white/90" : "text-slate-600"}>{feature}</span>
                    </div>
                  ))}
                </div>
              </div>
      ...
      <section className="py-20 px-4 sm:px-6 lg:px-8 bg-blue-600">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <p className="text-xl text-blue-100 mb-8">Join thousands of businesses already using Nealika POS</p>
          <button
            onClick={handleGetStarted}
          ...
          <p className="text-blue-100 mt-4">No credit card required • Cancel anytime</p>
        </div>
      </section>
